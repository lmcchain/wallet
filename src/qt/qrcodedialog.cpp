...
{
    QString ret = QString("lmccoin:%1").arg(address);
    int paramCount = 0;
    ...
    if (ui->chkReqPayment->isChecked())
    {
        ...
        {
            // even if we allow a non lmc unit input in lnReqAmount, we generate the URI with BKC as unit (as defined in BIP21)
